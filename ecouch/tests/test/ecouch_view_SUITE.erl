...
test_view_creation(_Config) -> 
    ?line {ok, {obj, [{"ok", true}, {"id", Id}, {"rev", Rev}]}} = 
	ecouch:view_create("ecouch_ct_test", "creation", 
			   {obj, [{"all", {obj, [{map, <<"function(doc){ emit(null, doc) }">>}]}}]}),
    ?line {ok, {obj, [{"_id", Id},
                      {"_rev", Rev},
                      {"language", <<"javascript">>},
...
test_view_access(_Config)->
    ?line {ok, _JsonObj} = 
	ecouch:view_create("ecouch_ct_test", "access", 
			   {obj, [{"all", {obj, [{map, <<"function(doc){ emit(null, doc); }">>}]}}]}),
    ?line {ok, {obj, [{"total_rows",2},{"offset", 0},{"rows",[_Row0, _Row1]}]}} = 
	ecouch:view_access("ecouch_ct_test", "access", "all", [{limit, 11}]).
