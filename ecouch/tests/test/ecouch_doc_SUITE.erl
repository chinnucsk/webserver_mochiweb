...
test_doc_creation_without_id(_Config) ->
	...
	ecouch:doc_create("ecouch_ct_test", {obj, [{"foo", 1}]}),
...
test_doc_get(_Config) ->
    ...
    ?line {ok, {obj, [{"_id", <<"foo_has_two">>}, {"_rev", _}, {"foo", 2}, {"bar", 3}]}} = 
	ecouch:doc_get("ecouch_ct_test", "foo_has_two").
...
test_doc_non_existing(_Config) ->
    ?line {ok, {obj, [{"error", <<"not_created">>}, {"reason", <<"invalid_name">>}]}} = 
	ecouch:doc_create("eouch_ct_test", "", {obj, []}),
    ?line {ok, {obj, [{"error", <<"not_deleted">>}, {"reason", <<"invalid_name">>}]}} = 
	ecouch:doc_delete("eouch_ct_test", "", "rev"),
    ?line {ok, {obj, [{"error", Error}, {"reason", _Reason}]}} = 
	ecouch:doc_get("ecouch_ct_test", "dummy"), 
    ?line {ok, {obj, [{"error", Error}, {"reason", <<"invalid_name">>}]}} = 
	ecouch:doc_get("ecouch_ct_test", ""). 
